...
def he_init(seed=0):
    return keras.initializers.he_normal(seed)
...
def script(n=3, version=2, seed=0, data_augmentation=False, epochs=200, model_index=None):
    ...
    save_dir = os.path.join(utils.SAVED_MODELS_DIR)
    ...
    save_subdir = os.path.join(save_dir, model_index)
    if model_index:
        if not os.path.isdir(save_subdir): os.makedirs(save_subdir)

        # Save the model
        model.save(os.path.join(save_subdir, model_name))

        # Also save the data
        filepath = os.path.join(save_subdir, "xy_train.npz")
    ...
    if model_index:
        model.save(os.path.join(save_dir, model_name + model_index + "_trained"))
